...
from generator.cfg import (
    CFG, CFGRepr, Node, Arrow, Loop, Ellipsis, Text, Vertex)
...
class Generator():
    ...
    def an(self, point, id_, l="s", t=False, f=True):
        self.graph.add(Node(point, name=l, index=id_, is_terminal=t,
            is_feasible=f))

    def ant(self, point, id_, l="s"):
        self.graph.add(Node(point, name=l, index=id_, is_terminal=True))

    def aa(self, point1, point2, f=True):
        self.graph.add(Arrow(point1, point2, is_feasible=f))

    def al(self, point, angle):
        self.graph.add(Loop(point, angle))

    def el(self, point):
        self.graph.add(Ellipsis(point, np.array((1, 0))))

    def draw(self, name, size: np.array = np.array((50, 50))):
        file_name: str = os.path.join(self.directory_name, name + ".svg")
        self.graph.draw(file_name, np.array((5, 5)) + size * 5)
        self.file_names.append(file_name)

    def line(
            self, graph: CFGRepr, x: float, y: float, name: str, indexes: list,
            step: float = 7.0, is_vertical: bool = False,
        ...
        for i, index in enumerate(indexes):
            if index == "...":
                ...
                graph.add(Node(np.array((x, y)), name=name, index=index,
    ...
    def generate(self):
        ...
        self.graph = CFGRepr(12.5)

        def se(x, y, vstep, hstep, level, i):
            self.an(v(x, y), i, l="s")
            self.aa(v(x, y), v(x + hstep, y - vstep))
            self.aa(v(x, y), v(x + hstep, y + vstep))
            self.aa(v(x, y), v(x + hstep, y))
            self.graph.add(Ellipsis(v(x + hstep, y), v(0, 1)))
            if level == 2:
                return
        ...
        self.graph.add(Text(svgwrite.text.Text("CFG", np.array((2.5, 2.5)) + np.array((x, y)) * 5)))
        self.line(self.graph, x, y + 4, "s", ["0", "1"], 5, True, True, True)

        x += 12
        self.graph.add(Text(svgwrite.text.Text("symbolic", np.array((2.5, 2.5)) + np.array((x, y - 2)) * 5)))
        self.graph.add(Text(svgwrite.text.Text("execution tree", np.array((2.5, 2.5)) + np.array((x, y)) * 5)))
        self.line(self.graph, x, y + 4, "s", ["0", "1"], 5, True, True)

        x += 12
        text_wrap = svgwrite.text.Text("", np.array((2.5, 2.5)) + np.array((x, y)) * 5)
        text_wrap.add(svgwrite.text.TSpan("P", font_style="italic"))
        text_wrap.add(svgwrite.text.TSpan(
            "0", font_size="65%", baseline_shift="sub"))
        self.graph.add(Text(text_wrap))
        self.line(self.graph, x, y + 4, "s", ["0", "1"], 5, True, True)
        ...
        self.an(v0, "0"); self.ant(v1, "1"); self.ant(v2, "2")
        self.aa(v0, v1); self.aa(v0, v2); self.al(v1, a3); self.al(v2, a3)

        x += 10
        v0, v1 = v(x, y), v(x, y + 5)
        self.an(v0, "0"); self.ant(v1, "1"); self.aa(v0, v1)

        x += 5
        v0, v1 = v(x, y), v(x, y + 5)
        self.an(v0, "0"); self.ant(v1, "2"); self.aa(v0, v1)

        x += 10
        v0, v1, v2 = v(x, y), v(x - 4, y + 4), v(x + 4, y + 4)
        self.an(v0, "0"); self.ant(v1, "1"); self.ant(v2, "2"); self.aa(v0, v1); self.aa(v0, v2)
        ...
        v0, v1 = v(x, y), v(x, y + 5)
        self.an(v0, "0"); self.ant(v1, "1")
        self.aa(v0, v1); self.al(v1, a3); self.al(v0, 0)
        ...
        self.an(v0, "0"); self.an(v1, "0"); self.ant(v2, "1"); self.aa(v0, v1); self.aa(v1, v2)

        x += 5
        v0, v1, v2, v3 = v(x, y), v(x, y + 5), v(x, y + 10), v(x, y + 15)
        self.an(v0, "0"); self.an(v1, "0"); self.an(v2, "0"); self.ant(v3, "1")
        self.aa(v0, v1); self.aa(v1, v2); self.aa(v2, v3)

        x += 10
        v0, v1, v2 = v(x, y), v(x - 4, y + 4), v(x + 4, y + 4)
        for i in range(3):
            self.an(v0, "0"); self.ant(v1, "1"); self.aa(v0, v1); self.aa(v0, v2)
        ...
        for i in range(4):
            self.an(v0, str(i * 2)); self.an(v1, str(i * 2 + 1))
            self.aa(v0, v1); self.aa(v0, v2); self.aa(v1, v2)
            v0 = v0 + v(0, 8)
            v1 = v1 + v(0, 8)
            v2 = v2 + v(0, 8)
        self.ant(v0, "8")
        ...
        def dr(index, x, y, step, count, f):
            self.an(v(x, y), str(index), t=index == 8, f=f)
            if index == 8:
                return
            if index in [1, 3, 5]:
                count += 1
            if index % 2 == 0:
                self.aa(v(x, y), v(x + 7, y - step), f=count != 3)
                dr(index + 2, x + 7, y - step, step / 2.0, count, f=count != 3)
                self.aa(v(x, y), v(x + 7, y + step), f=index < 6 or count == 3)
                dr(index + 1, x + 7, y + step, step / 2.0, count,
                    f=index < 6 or count == 3)
            else:
                self.aa(v(x, y), v(x + 7, y), f=index < 6 or count == 3)
        ...
        for i in range(4):
            self.an(v0, str(i * 2)); self.an(v1, str(i * 2 + 1))
            self.aa(v0, v1); self.aa(v0, v3); self.aa(v1, v2)
            v0 = v0 + v(4, 9)
            v1 = v1 + v(4, 9)
            v2 = v2 + v(4, 9)
        self.an(v0, "8")
        ...
        with open("result.html", "w+") as debug_file:
            debug_file.write("".join(map(
                lambda x: f"<img src=\"{x}\">", self.file_names)))
