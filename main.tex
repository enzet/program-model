...
\title{Formal model of program execution, symbolic,
...
We want to start from the classical definition of a
...
where $f: Q \ra Q$, $Q$~--- states\footnote{
    Note, that there is no requrement, that all states should be reachable.
    We can have state $q \in Q$ that is not initial and
    for all initial states $q$ is not in it's computation.
}, $\Omega$~--- terminal states,
$I$~--- input states.
In this model execution of a computational method is
$\forall x \in I \,\, x_0, x_1, \dots, x_k, \dots$.
$x_0 = x, x_{k + 1} = f(x_k)$.
...
$\mathcal{P} = \langle \mathcal{F}, D, D_I, D_T \rangle$,
where $\mathcal{F}: D \ra D$ is an \term{execution function},
$D$~--- states, $D_I$~--- initial states,
$D_T$~--- terminal states.

This definition is concise and theoretically
...
Now for each set $s_i$ we can define function $f_i$.
...
These functions we will call \altterm{function elements}{function element}.
...
For the sake of disambiguity, from now for the first meaning we will use term
\term{process state}, and for the second---\term{program state},
and will never use the term \textit{state} without a specifier.
For the process state we will use small letter $d$,
for the set of process states---capital letter $D$,
for the program state---small letter $s$\footnote{
...
\centerimage{states}{Process state and program state specifiers.}

For the sake of convenience, we will require that $s_0 = D_I$
so we can treat $s_0$ as an initial program state
and that there is a superset $\{s_j\}$ that form $D_T$
so we can treat them as terminal program states: $D_T = \bigcup_{j} s_j$.

Now we can move our partition of a state set
propose another definition of the program:
$\mathcal{P} = \langle F, D, S, s_0, S_T \rangle$,
where
\begin{itemize}
    \item $F = \{f_i\}, f_i: D \ra D$ is a set of function elements,
    \item $D$ is a set of all possible process states,
    \item $S = \{s_j\}$ is a set of program states,
    \item $s_0$ is a initial program state,
    \item $S_T = \{s_k\}$ is a set of terminal program states.
\end{itemize}

Note, that current program state defines the next function element to be
used to proceed program execution.

Here $F$ can be treated as a \term{instruction set},
set of operations, or set of possible \altterm{statements}{statement}.

\subsublevel*{Split data flow and control flow}

Assume, that we can divide every process state $d_i$ into two parts
    $d_i = (d^D_i, d^F_i)$.
So now there are two sets $D^D = \{d^D_i\}$ and 
    $D^F = \{d^F_i\}$.
Assume also, that for every $f_i$
    there is a function $f^F_i$ such as
    $d_i \in s_j \Lra f^F(d^F_i) = j$ and
    and function $f^D_i$ such as
    $(d^D_k, d^F_k) = f_t\big((d^D_i, d^F_i)\big) \Lra
        d^D_k = \varphi_t(d^D_i)$.

Then we will call $d^D_i$ a \term{data element} and $d^D$ a \term{data},
and $d^F_i$ a \term{program state designator}.
It can be treated as \term{instruction pointer} or \term{program counter}.

As was said before, current program state defines the next function element.
Therefore we can
define initial function element instead of initial program state,
terminal function elements instead of terminal program states,
and remove program state designator from the process state,
but make our function elements return new process state \bftext{and}
