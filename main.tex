...
\documentclass[oneside,a4]{article}  % fleqn
...
\begin{enumerate}
    \item state of progress, state of the system, state formula,
    \item record of \textit{m}-configuration, value of state register,
          designator of instruction.
\end{enumerate}
In the Knuth's definition the term state is used in the first meaning.
...
$D = D_0 \cup D_1 \cup \dots \cup D_m,
    \forall i, j \,\, i \neq j \Ra D_i \cap D_j = \varnothing$.
Now for each set $D_i$ we can define function $f_i$.
$\forall d \in S_i \,\, f_i(d) = \mathcal{F}(d)$.

\begin{align}
\mathcal{F}(d) =
    \begin{cases}
        f_0(d), & \text{if } d \in D_0, \\
        f_1(d), & \text{if } d \in D_1, \\
        \dots \\
        f_m(d), & \text{if } d \in D_m. \\
    \end{cases}
\end{align}
...
Also, we can treat sets $D_i$ as program \altterm{states}{state}.

For the sake of convenience, we will require that $D_0 = D_I$
so we can treat $D_0$ as an initial state
and that there is a superset $\{D_j\}$ that form $D_T$
so we can treat them as terminal states: $D_T = \bigcup_{j} D_j$.

Now we can move our partition of a data set propose another definition of the program:
$\mathcal{P} = \langle F, D, \mathcal{D}, D_I, \mathcal{D}_T \rangle$,
where
\begin{itemize}
    \item $F = \{f_i\}, f_i: D \ra D$ is a set of function elements,
    \item $D$ is a data set,
    \item $\mathcal{D} = \{D_j\}$ is a superset of states,
    \item $D_I = D_0$ is a initial state,
    \item $\mathcal{D}_T = \{D_k\}$ is a superset of terminal states.
\end{itemize}
