...
For each result $d^i$ we can create another function $\phi_i$,
that returns $d^i$ if its argument is $d_i$ and anything otherwise.

\begin{align}
\mathcal{F}(d) =
    \begin{cases}
        \phi_1(d), \, \text{if } d = d_1, \\
        \phi_2(d), \, \text{if } d = d_2, \\
        \dots \\
        \phi_n(d), \, \text{if } d = d_n. \\
    \end{cases}
\end{align}
...
\mathcal{F}(d) =
    \begin{cases}
        ...
        f_2(d), \, \text{if } d \in D_2, \\
        \dots \\
        f_n(d), \, \text{if } d \in D_n. \\
    \end{cases}
\end{align}

Now we can say that our program has a number of possible states:
$s_1 = D_1, s_2 = D_2, s_3 = D_3$.
