...
\lstset{basicstyle=\linespread{0.9}\small\ttfamily,keywordstyle=\bfseries\color{black},stringstyle=\color{black},identifierstyle=\color{black}}
...
The main goal of this document is to offer a formal definition of
some program analysis terms, such as symbolic execution,
dynamic symbolic execution, feasibility, and taint analysis.
...
\begin{itemize}
    ...
    \item $\bot$ is a special terminal symbol.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item If $f_j \notin F_T$,
            $\forall d \in D \,\, f^F_j(d) = f_k \neq \bot, f_k \in F$.
        \item If $f_j \in F_T$,
            $\forall d \in D \,\, f^F_j(d) = \bot$.
    \end{itemize}
    \item To simplify considering, we will assume that unless otherwise stated
        ...
        $F_I = \{f_0\}$.
    \item Program has terminal function elements: $F_T \neq \varnothing$.
\end{itemize}
...
$s_t = (f_t, d_t) = \big((f_t^F, f_t^D), (\{m_{t,j}\}, \{r_{t,j}\})\big)$.
...
$$\mathcal{F}(s_t) = \mathcal{F}\big((f_t, d_t)\big) =
    (f_t^F(d_t), f_t^D(d_t)) = (f_{t + 1}, d_{t + 1}) = s_{t + 1}.$$
...
We will denote this fact as $E(d_0) \leadsto P$.
...
$\mathcal{P} = \langle F, F_I, F_T, D \rangle$.
We assume, that there is a set of rules $R_\text{SE}$ that allow us to
create a symbolic model from the program.
We can write it as
...
From definition:
\begin{itemize}
    \item $\se{F} = \{\se{f}_j\} = \{(\se{f}_j^F, \se{f}_j^D)\}$~---
    ...
    \begin{itemize}
        \item $\se{f}_j^F: \se{D} \ra \se{F}*$~---
            \term{control flow symbolic function element model},
        \item $\se{f}_j^D: \se{D} \ra \se{D}*$~---
            \term{data symbolic function element model}.
    \end{itemize}
    % \item $\se{F} = \{(\se{f}_j^F, \se{f}_j^D)\}$~---
    %     concolic function element models.
    % \begin{itemize}
    %     \item $\se{f}_j^F: \se{D} \times D \ra \se{F}$.
    %     \item $\se{f}_j^F: \se{D} \times D \ra \se{D}$.
    % \end{itemize}
\end{itemize}
...
Definitions:
\begin{itemize}
    \item $\se{S} = (\se{f}, \se{d}, c)$~--- \term{symbolic state}.
          $\se{f} \in \se{F}, \se{d} \in \se{D}, c \in C$.
    \item $C = \{e_i\}$~--- \term{path condition}.
...
Execution path $P = \{f_0, f_1, \dots, f_t\}$ is
