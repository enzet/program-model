...
Set $D$ we will also call \term{data}.

 \comment{Connection between our definitions and real-world entities.}

\subsublevel*{Program state designator}

If we can divide every process state $d_i$ into two parts
$d_i = (a_i, b_i)$ in such a way that we can propose a function
$g$ with the next feature:
$d_i \in s_j \Lra g(b_i) = j$,
then we will call $b_i$ a \term{program state designator}.
