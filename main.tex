...
Classical program definition could be found in
Donald Knuth's \textit{The Art of Computer Programming}~\cite{knuth}:
\term{program} is $\langle Q, I, \Omega, f \rangle$,
...
It is convenient to use slightly different notation for further considerations:
$\mathcal{P} = \langle \mathcal{F}, D_I, D_T, D \rangle$,
where $\mathcal{F}: D \ra D$, $D$~--- states, $D_I$~--- initial states,
$D_T$~--- terminal states.

\subsublevel*{Split function elements}
...
as a sequence of variables $X = (x_j)$.
And let us symboilc data model $\se{D}$ be a
