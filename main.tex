...
\begin{abstract}
\en{The main goal of this document is to propose a formal definition of
...
\level{
    \en{Concrete program execution}
    \ru{Фактическое исполнение программы}
}

\sublevel{
    \en{Program definition}
    \ru{Определение программы}
}
...
\begin{align}\mathcal{P} = \langle \mathcal{F}, D, D_I, D_T \rangle,\end{align}
...
computer systems,
so we will expand this definition with more complex constructions.}
\noru{}

\subsublevel*{
    \en{Split execution function}
    \ru{Разделение функции исполнения}
}
...
\en{We can split set $D$ into a number of disjoint sets:
...
\en{It is very importaint to emphasize, that we defined function elements and
...
\begin{align}\mathcal{P} = \langle F, D, S, s_0, S_T \rangle,\end{align}
where
...
\begin{align}\mathcal{P} = \langle F, D, S, f_0, F_T \rangle\end{align}
where
...
\subsublevel*{
    \en{Possible data model definition}
...
\en{However it is convenient to use more complicated data definitions to simplify
modeling.}
\noru{}
